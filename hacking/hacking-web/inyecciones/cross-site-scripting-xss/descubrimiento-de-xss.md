...
Almost all Web Application Vulnerability Scanners (like [Nessus](https://www.tenable.com/products/nessus), [Burp Pro](https://portswigger.net/burp/pro), or [ZAP](https://www.zaproxy.org/)) have various capabilities for detecting all three types of XSS vulnerabilities. These scanners usually do two types of scanning:
...
While paid tools usually have a higher level of accuracy in detecting XSS vulnerabilities (especially when security bypasses are required), we can still find open-source tools that can assist in identifying potential XSS vulnerabilities. Such tools usually work by identifying input fields in web pages, sending various types of XSS payloads, and then comparing the rendered page source to see if the same payload can be found in it — which may indicate a successful XSS injection. Still, this will not always be accurate, as sometimes, even if the same payload was injected, it might not lead to successful execution due to various reasons, so we must always manually verify the XSS injection.

Some common open-source tools that can assist in XSS discovery are [XSS Strike](https://github.com/s0md3v/XSStrike), [Brute XSS](https://github.com/rajeshmajumdar/BruteXSS), and [XSSer](https://github.com/epsylon/xsser). We can try XSS Strike by cloning it to our VM with `git clone`:

{% code title="XSS Discovery" %}
```shell-session
OsmanMartinez@htb[/htb]$ git clone https://github.com/s0md3v/XSStrike.git
OsmanMartinez@htb[/htb]$ cd XSStrike
OsmanMartinez@htb[/htb]$ pip install -r requirements.txt
OsmanMartinez@htb[/htb]$ python xsstrike.py
...
We can then run the script and provide it a URL with a parameter using `-u`. Example using our `Reflected XSS` example:

{% code title="XSS Discovery" %}
```shell-session
OsmanMartinez@htb[/htb]$ python xsstrike.py -u "http://SERVER_IP:PORT/index.php?task=test" 
...
{% hint style="info" %}
XSS can be injected into any input in the HTML page, which is not exclusive to HTML input fields, but may also be in HTTP headers like Cookie or User-Agent (i.e., when their values are displayed on the page).
{% endhint %}
...
This is why it is not very efficient to resort to manually copying/pasting XSS payloads, as even if a web application is vulnerable, it may take a while to identify the vulnerability, especially if there are many input fields to test. It may be more efficient to write a custom script (for example, in Python) to automate sending these payloads and then comparing the page source to see how our payloads were rendered. This can help in advanced cases where XSS tools cannot easily send and compare payloads. However, this is an advanced approach to XSS discovery and is not part of the scope of this module.

<details>

<summary>External payload resources</summary>
...
These are advanced techniques that are out of scope for this module. If you're interested in learning them further, see the referenced modules below:
